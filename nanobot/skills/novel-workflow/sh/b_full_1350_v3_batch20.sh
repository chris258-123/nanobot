...
LOG=/home/chris/Desktop/my_workspace/novel_data/04/new_book/log/b_full_1350_v3_batch20.log
mkdir -p "$(dirname "$LOG")"

echo "[$(date +"%F %T")] ==== START full rerun book=novel_04_b_full_1350_v3 range=1-1350 batch=20 ====" | tee -a "$LOG"

for s in $(seq 1 20 1350); do
  e=$((s+19)); [ $e -gt 1350 ] && e=1350
  ok=0
  ...
  for a in 1 2 3; do
    ...
    /home/chris/miniforge3/envs/nanobot/bin/python /home/chris/Desktop/my_workspace/nanobot/nanobot/skills/novel-workflow/scripts/generate_book_ab.py \
      --target-book-id novel_04_b_full_1350_v3 \
      ...
      --chapter-count $((e-s+1)) \
      --start-chapter $s \
      --output-dir /home/chris/Desktop/my_workspace/novel_data/04/new_book/full_1350_v3 \
      ...
      --target-canon-db-path /home/chris/Desktop/my_workspace/novel_data/04/new_book/canon_novel_04_b_full_1350_v3.db \
      --target-neo4j-uri bolt://localhost:7693 \
      --target-neo4j-user neo4j \
      --target-neo4j-pass novel123 \
      --target-neo4j-database neo4j \
      --target-qdrant-url http://localhost:6333 \
      --target-qdrant-collection novel_04_b_full_1350_v3_assets \
